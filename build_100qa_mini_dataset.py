#!/usr/bin/env python3
"""
Build a mini ECG-QA dataset with 100 randomly sampled QA pairs from MIMIC-IV-ECG.
...
# Load the sampled ECG IDs
with open('sampled_ecgs_for_100qa.txt', 'r') as f:
    sampled_ecg_ids = set(int(line.strip()) for line in f)

print(f"Sampled {len(sampled_ecg_ids)} ECG IDs")
...
print(f"Total paraphrased samples: {len(all_paraphrased)}")
...
print(f"Total template samples: {len(all_template)}")

# Filter samples: all ECG IDs must be downloaded AND in sampled set
def filter_samples(samples, sampled_ids, downloaded_ids):
    """Filter samples to only include those with all ECG IDs downloaded and sampled."""
    filtered = []
    for sample in samples:
        ecg_ids = sample['ecg_id']
        if (all(ecg_id in downloaded_ids for ecg_id in ecg_ids) and
            all(ecg_id in sampled_ids for ecg_id in ecg_ids)):
            filtered.append(sample)
    return filtered

print("\nFiltering samples...")
filtered_paraphrased = filter_samples(all_paraphrased, sampled_ecg_ids, downloaded_study_ids)
filtered_template = filter_samples(all_template, sampled_ecg_ids, downloaded_study_ids)
...
print(f"\nQuestion type distribution (paraphrased):")
for qt, count in sorted(question_types.items()):
    print(f"  {qt}: {count}")

print(f"{'='*60}")
print(f"\nDataset structure matches ECG-QA PTB-XL format:")
...
print(f"  - paraphrased/train/*.json (paraphrased questions)")
print(f"  - template/train/*.json (template questions)")
