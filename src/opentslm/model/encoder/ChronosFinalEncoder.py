...
"""
Final working Chronos encoder with ALL device issues fixed.
...
class ChronosFinalEncoder(TimeSeriesEncoderBase):
    """
    Final working Chronos encoder with complete device handling.

    This properly loads and uses Chronos models from HuggingFace with:
    - All tensors on correct device
    - Proper handling of tokenizer components
    ...
    def __init__(
        ...
        # Load Chronos pipeline on CPU first
        ...
        # CRITICAL: Move ALL tokenizer tensors to the target device
        if hasattr(self.pipeline.tokenizer, 'boundaries'):
            self.pipeline.tokenizer.boundaries = self.pipeline.tokenizer.boundaries.to(self.device)
        if hasattr(self.pipeline.tokenizer, 'centers'):
            self.pipeline.tokenizer.centers = self.pipeline.tokenizer.centers.to(self.device)
        ...
        # Create projection layer on correct device
        ...
        print(f"ChronosFinalEncoder initialized: {encoder_hidden_dim} -> {output_dim}")
    ...
    def get_model_name(self) -> str:
        return f"ChronosFinal-{self.model_name.split('/')[-1]}"