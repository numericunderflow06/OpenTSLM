...
"""
Final working PatchTST encoder with correct stride handling.
...
class PatchTSTFinalEncoder(TimeSeriesEncoderBase):
    """
    Final working PatchTST encoder with correct patch stride.

    This properly loads pretrained PatchTST models and works around
    the patchifier bug with correct stride calculation.
    ...
    def __init__(
        ...
        # CRITICAL FIX: patch_stride should be same as patch_length for non-overlapping patches
        # The config has patch_stride=1 which is wrong for the pretrained model
        self.patch_stride = self.patch_length  # Use non-overlapping patches

        print(f"  Expected: {self.num_input_channels} channels, {self.context_length} sequence length")
        print(f"  Patch length: {self.patch_length}, stride: {self.patch_stride} (fixed)")
        ...
        # Projection layer on correct device
        ...
        print(f"PatchTSTFinalEncoder initialized: {model_output_dim} -> {output_dim}")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad() if self.freeze_backbone else torch.enable_grad():
            ...
            # Manually create patches with correct stride
    ...
    def get_model_name(self) -> str:
        return f"PatchTSTFinal-{self.model_name.split('/')[-1]}"