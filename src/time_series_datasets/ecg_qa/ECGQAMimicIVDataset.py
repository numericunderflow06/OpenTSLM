...
class ECGQAMimicIVDataset(QADataset):
    """
    ECG-QA MIMIC-IV Dataset for question answering with electrocardiogram data.

    This dataset combines ECG time series data from MIMIC-IV-ECG with
    question-answer pairs from the ECG-QA MIMIC-IV dataset.

    Compatible with ECGQACoTQADataset for easy cross-dataset testing.
    ...
    def __init__(self, split: Literal["train", "test", "validation"], EOS_TOKEN: str,
                 ...
                 preload_processed_data: bool = True, use_cot_format: bool = True):
        """
        Initialize ECG-QA MIMIC-IV Dataset.
        ...
        Args:
            ...
            use_cot_format: If True, use chain-of-thought format (rationale as answer). Default: True for compatibility with PTB-XL CoT.
        """
        self.max_samples = max_samples
        self.exclude_comparison = exclude_comparison
        self.preload_processed_data = preload_processed_data
        self.use_cot_format = use_cot_format
        super().__init__(split, EOS_TOKEN, format_sample_str, time_series_format_function)

    def _load_splits(self) -> Tuple[Dataset, Dataset, Dataset]:
        """Load the ECG-QA MIMIC-IV dataset splits."""
        print("Loading ECG-QA MIMIC-IV dataset splits...")
    ...
    def _get_answer(self, row) -> str:
        ...
        if self.use_cot_format:
            # Use chain-of-thought format: create a rationale that leads to the answer
            answer = row.get("answer", "")
            question = row.get("question", "")

            # For MIMIC-IV, we don't have pre-generated rationales, so create a simple format
            # that mimics CoT structure for compatibility
            rationale = f"Based on the ECG analysis, the answer is: {answer}"
            return rationale
        else:
            # Just return the answer directly
            return row.get("answer", "No answer available.")
    ...
    def _get_pre_prompt(self, row) -> str:
        ...
        if self.use_cot_format:
            # Use CoT format for compatibility with PTB-XL CoT training
...
Please analyze the ECG signal and provide your answer."""
    ...
    def _get_post_prompt(self, row) -> str:
        ...
        possible_answers = ECGQAMimicIVDataset.get_possible_answers_for_template(template_id)
...
"""
    ...
    def _load_template_answers_cache(cls):
        ...
        if cls._template_answers_cache is None:
            try:
                import pandas as pd
                import ast
                from time_series_datasets.ecg_qa.mimiciv_ecg_loader import ECG_QA_DIR

                # Load template answers for MIMIC-IV
                template_answers_path = os.path.join(ECG_QA_DIR, "ecgqa", "mimic-iv-ecg", "answers_for_each_template.csv")

                if not os.path.exists(template_answers_path):
                    print(f"Warning: Template answers file not found at {template_answers_path}")
                ...
                print(f"Loaded MIMIC-IV template answers cache with {len(cls._template_answers_cache)} templates")
    ...
    def get_possible_answers_for_template(template_id: int) -> List[str]:
        ...
        ECGQAMimicIVDataset._load_template_answers_cache()

        # Return cached result
        return ECGQAMimicIVDataset._template_answers_cache.get(template_id, [])
    ...
    def get_labels() -> List[str]:
        ...
        try:
            ...
            print(f"Loaded {len(labels)} unique labels from MIMIC-IV answers_for_each_template.csv")
            return labels
        except Exception as e:
            print(f"Error loading labels from CSV: {e}")
            return ["yes", "no", "not sure", "none", "normal", "abnormal"]

    # Class-level cache for ECG data (shared with PTB-XL for efficiency)
    _ecg_data_cache = {}
    ...
    def _load_ecg_data(cls, ecg_path: str) -> Tuple[np.ndarray, str, str]:
        ...
        if ecg_path not in cls._ecg_data_cache:
            try:
                ...
                # MIMIC-IV-ECG is sampled at 500Hz, 10 seconds = 5000 samples
                # We'll downsample to 100Hz like PTB-XL for consistency
    ...
    def preload_ecg_data(cls, dataset_splits: List[Dataset]):
        ...
        print("Preloading MIMIC-IV ECG data for faster formatting...")
        ...
        print(f"Found {len(ecg_paths)} unique MIMIC-IV ECG files to preload...")

        # Preload ECG data with progress bar
        from tqdm import tqdm
        for ecg_path in tqdm(ecg_paths, desc="Preloading MIMIC-IV ECG data"):
            try:
                cls._load_ecg_data(ecg_path)
            except Exception as e:
                print(f"Warning: Failed to preload ECG data from {ecg_path}: {e}")

        print(f"Preloaded {len(cls._ecg_data_cache)} MIMIC-IV ECG files")
    ...
    def preload_processed_ecg_data(cls, dataset_splits: List[Dataset]):
        ...
        print("Preloading processed MIMIC-IV ECG data for maximum performance...")
                ...
                if sample_ecg_paths:
                    for ecg_path in sample_ecg_paths:
                        ...
                        try:
                            ...
                            print(f"Warning: Failed to preload ECG for preprocessing from {ecg_path}: {e}")

        print(f"Found {len(ecg_lead_combinations)} unique MIMIC-IV ECG lead combinations to preprocess...")

        # Preprocess ECG leads with progress bar
        from tqdm import tqdm
        for ecg_path, lead_idx in tqdm(ecg_lead_combinations, desc="Preprocessing MIMIC-IV ECG leads"):
            try:
                cls._process_ecg_lead(ecg_path, lead_idx)
            except Exception as e:
                print(f"Warning: Failed to preprocess ECG lead {lead_idx} from {ecg_path}: {e}")

        print(f"Preprocessed {len(cls._processed_ecg_cache)} MIMIC-IV ECG leads")
    ...
    def clear_caches(cls):
        ...
        print("Cleared all ECG-QA MIMIC-IV dataset caches")
    ...
    def _process_ecg_lead(cls, ecg_path: str, lead_idx: int) -> Tuple[np.ndarray, float, float]:
        ...
        if cache_key not in cls._processed_ecg_cache:
            ...
            # Downsample if needed (MIMIC-IV is 500Hz, downsample to 100Hz)
            if len(lead_signal) > 1000:  # Likely 500Hz data (5000 samples for 10 seconds)
                downsampled_signal = lead_signal[::5]  # Downsample to 100Hz (1000 samples)
    ...
    def _get_text_time_series_prompt_list(self, row) -> List[TextTimeSeriesPrompt]:
        ...
        if not ecg_paths:
            raise ValueError(f"Sample has empty ecg_paths list: {row}")
    ...
    def _format_sample(self, row):
        ...
        # Store the ground-truth answer and possible answers for this template
        if 'answer' in row:
            formatted_sample['correct_answer'] = row['answer']
        if 'template_id' in row and row['template_id'] is not None:
            try:
                formatted_sample['possible_answers'] = ECGQAMimicIVDataset.get_possible_answers_for_template(row['template_id'])
            except Exception:
...
if __name__ == "__main__":
    ...
    print("Testing ECGQAMimicIVDataset...")

    try:
        # Test with just 2 samples per split for faster testing
        dataset = ECGQAMimicIVDataset(split="train", EOS_TOKEN="", max_samples=2, preload_processed_data=False)
        dataset_val = ECGQAMimicIVDataset(split="validation", EOS_TOKEN="", max_samples=2, preload_processed_data=False)
        dataset_test = ECGQAMimicIVDataset(split="test", EOS_TOKEN="", max_samples=2, preload_processed_data=False)

        print(f"Dataset sizes: Train: {len(dataset)}, Validation: {len(dataset_val)}, Test: {len(dataset_test)}")

        # Show cache statistics
        cache_stats = ECGQAMimicIVDataset.get_cache_stats()
        ...
        if len(dataset) > 0:
            ...
            print("Sample answer:", sample["answer"][:200] + "..." if len(sample["answer"]) > 200 else sample["answer"])
