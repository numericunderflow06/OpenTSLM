#
# ZuCo EEG Reading Task Classification Dataset for OpenTSLM
#
...
from time_series_datasets.zuco_eeg.zuco_eeg_loader import load_zuco_eeg_reading_task_splits
import torch
import numpy as np
...
class ZuCoEEGReadingTaskDataset(QADataset):
    def __init__(
        self,
    ...
    def _load_splits(self) -> Tuple[Dataset, Dataset, Dataset]:
        return load_zuco_eeg_reading_task_splits()
    ...
    def _get_pre_prompt(self, row) -> str:
        return (
            "You are given multi-channel EEG signals recorded during reading. "
...
if __name__ == "__main__":
    print("=== ZuCo EEG Reading Task Dataset ===\n")

    dataset = ZuCoEEGReadingTaskDataset(split="train", EOS_TOKEN="")
    dataset_val = ZuCoEEGReadingTaskDataset(split="validation", EOS_TOKEN="")
    dataset_test = ZuCoEEGReadingTaskDataset(split="test", EOS_TOKEN="")
    ...
    if len(dataset) > 0:
        ...
        # Check time series info
        ts_list = sample.get("time_series_list", [])
        if ts_list:
            print(f"\nNumber of time series channels: {len(ts_list)}")
            for i, ts in enumerate(ts_list):
                print(f"  Channel {i}: length={len(ts)}")
